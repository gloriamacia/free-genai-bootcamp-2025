...
def load(app):
  ...
  def get_group_words(id):
    try:
      ...
      sort_by = request.args.get('sort_by', 'catalan')
  ...
  def get_group_words_raw(id):
      try:
          ...
          # Query to fetch all words related to the group without pagination
          cursor.execute('''
          ...
          # Format response
          words_data = [{"id": word["id"], "catalan": word["catalan"], "english": word["english"]} for word in words]
